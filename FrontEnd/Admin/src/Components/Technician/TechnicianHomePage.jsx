...
function TechnicianHomePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-indigo-400/10 rounded-full blur-3xl -ml-48 -mb-48 animate-pulse" style={{ animationDelay: "2s" }}></div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-12 relative z-10">
        ...
        >
          ...
          >
            <div className="md:flex">
              <div className="md:w-1/3 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 p-10 flex flex-col items-center justify-center text-white">
                ...
                <h2 className="text-2xl font-bold mb-1 opacity-90">Welcome Back</h2>
                <p className="text-blue-100 text-center font-medium">#{technicianData?._id?.slice(-6)}</p>
              </div>
              
              <div className="md:w-2/3 p-10 space-y-8">
                <div>
                  <h3 className="text-3xl font-bold text-gray-900 mb-2">
                    Hello, <span className="text-blue-600">{technicianData?.userName}</span>!
                  </h3>
                  <p className="text-gray-500 font-medium italic">We're glad to have you back on duty today.</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-blue-200 hover:bg-blue-50/30 transition-all duration-300">
                    ...
                    <div>
                      ...
                      <p className="text-slate-700 font-bold capitalize">{technicianData?.userrole}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-indigo-200 hover:bg-indigo-50/30 transition-all duration-300">
                    ...
                    <div>
                      ...
                      <p className="text-slate-700 font-bold">{technicianData?.joinAt || "Dec 2023"}</p>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: CheckCircle2, label: "Status", value: "Active Now", color: "green" },
              { icon: Clock, label: "Current Shift", value: "Day Shift", color: "blue" },
              { icon: Zap, label: "Availability", value: "Instant Pick", color: "purple" }
            ].map((stat, idx) => (
              <motion.div
                key={idx}
                variants={itemVariants}
              ...
              >
                <div className={`w-16 h-16 bg-${stat.color}-100 rounded-2xl flex items-center justify-center mb-4 text-${stat.color}-600 group-hover:scale-110 transition-transform duration-300 shadow-inner`}>
                  <stat.icon className="w-8 h-8" />
                </div>
                <h4 className="text-gray-500 font-bold text-sm uppercase tracking-widest mb-1">{stat.label}</h4>
      ...
      <footer className="max-w-5xl mx-auto px-4 py-8 text-center border-t border-slate-200/50 mt-12 bg-white/20">
        <p className="text-slate-400 text-sm font-medium">
          &copy; {new Date().getFullYear()} Cool Services Pro. All secure data encrypted.
        </p>
      </footer>
    </div>
  );
